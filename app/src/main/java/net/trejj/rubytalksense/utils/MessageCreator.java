...
import net.trejj.rubytalksense.model.realms.RealmLocation;
...
public class MessageCreator {
    ...
    private static Message createTextMessage(User user, String text) {
        ...
        //messageId
        final String pushKey = net.trejj.rubytalksense.utils.FireConstants.messages.push().getKey();
    ...
    private static Message createImageMessage(User user, final String imagePath, boolean fromCamera) {
        ...
        File file = net.trejj.rubytalksense.utils.DirManager.generateFile(type);
        String fileExtension = net.trejj.rubytalksense.utils.Util.getFileExtensionFromPath(imagePath);

        if (fileExtension.equals("gif")) {
            try {
                net.trejj.rubytalksense.utils.FileUtils.copyFile(imagePath, file);
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
            //compress image and copy it to the given file
            net.trejj.rubytalksense.utils.BitmapUtils.compressImage(imagePath, file);
        }
        ...
        if (fromCamera) {
            ...
            net.trejj.rubytalksense.utils.FileUtils.deleteFile(imagePath);
        }
        ...
        //set the file size
        String fileSize = net.trejj.rubytalksense.utils.Util.getFileSizeFromLong(file.length(), true);

        message.setLocalPath(filePath);
        //blurred thumb image
        String thumb = net.trejj.rubytalksense.utils.BitmapUtils.decodeImage(filePath, true);
    ...
    private static Message createVideoMessage(Context context, User user, final String path) {
        ...
        //get video size
        String videoSize = net.trejj.rubytalksense.utils.Util.getFileSizeFromLong(file.length(), true);
        String receiverUid = user.getUid();


        //get raw image thumb bitmap
        Bitmap videoThumbBitmap = net.trejj.rubytalksense.utils.BitmapUtils.getThumbnailFromVideo(path);
        //generate blurred thumb to send it to other user
        String thumb = net.trejj.rubytalksense.utils.BitmapUtils.decodeImage(videoThumbBitmap);
        ...
        //set video duration
        message.setMediaDuration(net.trejj.rubytalksense.utils.Util.getVideoLength(context, path));
    ...
    private static Message createAudioMessage(User user, String filePath, String audioDuration) {
        ...
        String fileSize = net.trejj.rubytalksense.utils.Util.getFileSizeFromLong(audioFile.length(), true);
        //get file extension
        String fileExtension = net.trejj.rubytalksense.utils.Util.getFileExtensionFromPath(filePath);
        File file = net.trejj.rubytalksense.utils.DirManager.generateAudioFile(type, fileExtension);

        try {
            //copy original file to this new path
            net.trejj.rubytalksense.utils.FileUtils.copyFile(audioFile, file);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    private static Message createFileMessage(User user, final String filePath) {
        ...
        final Message message = new Message();
        final String pushKey = net.trejj.rubytalksense.utils.FireConstants.messages.push().getKey();
        final String fileName = net.trejj.rubytalksense.utils.Util.getFileNameFromPath(filePath);
    ...
    public static Message createForwardedMessage(Message mMessage, User user, String fromId) {
        ...
        net.trejj.rubytalksense.utils.RealmHelper.getInstance().saveObjectToRealm(message);
        net.trejj.rubytalksense.utils.RealmHelper.getInstance().saveChatIfNotExists(message, user);
        return message;
    }
        ...
        public Message build() {
            ...
            if (user.isBroadcastBool()) {
                ...
                for (User user : user.getBroadcast().getUsers()) {
                    ...
                    net.trejj.rubytalksense.utils.RealmHelper.getInstance().saveObjectToRealm(clonedMessage);
                    net.trejj.rubytalksense.utils.RealmHelper.getInstance().saveChatIfNotExists(clonedMessage, user);
                }
            }
