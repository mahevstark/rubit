...
import net.trejj.rubytalksense.activities.calling.CallingActivity;
import net.trejj.rubytalksense.activities.main.MainActivity;
import net.trejj.rubytalksense.activities.main.messaging.ChatActivity;
import net.trejj.rubytalksense.model.realms.Chat;
import net.trejj.rubytalksense.model.realms.FireCall;
import net.trejj.rubytalksense.model.realms.Message;
import net.trejj.rubytalksense.model.realms.User;
import net.trejj.rubytalksense.receivers.HandleReplyReceiver;
import net.trejj.rubytalksense.receivers.MarkAsReadReceiver;
...
import static net.trejj.rubytalksense.utils.IntentUtils.NOTIFICATION_ACTION_DECLINE;
import static net.trejj.rubytalksense.utils.MessageTypeHelper.getMessageContent;
...
public class NotificationHelper extends ContextWrapper {
    ...
    private Bitmap getProfilePhotoAsBitmap(String thumbImg) {
        if (thumbImg != null)
            return net.trejj.rubytalksense.utils.BitmapUtils.encodeImage(thumbImg);
    ...
    public void dismissNotification(String chatId, boolean decrementCount) {
        ...
        Chat chat = net.trejj.rubytalksense.utils.RealmHelper.getInstance().getChat(chatId);
        if (chat != null) {
            ...
            if (decrementCount) {
                ...
                net.trejj.rubytalksense.utils.RealmHelper.getInstance().deleteUnReadMessages(chatId);
            }
            //dismiss grouped notification if there are no notifications left
            if (!isBelowApi24() && !net.trejj.rubytalksense.utils.RealmHelper.getInstance().areThereUnreadChats()) {
    ...
    public void fireNotification(String newMessageChatId) {
        ...
        long unreadMessagesCount = net.trejj.rubytalksense.utils.RealmHelper.getInstance().getUnreadMessagesCount();
        ...
        if (isBelowApi24()) {
            ...
            List<Message> last7UnreadMessages = net.trejj.rubytalksense.utils.RealmHelper.getInstance().getLast7UnreadMessages();
            final NotificationCompat.MessagingStyle messagingStyle = new NotificationCompat.MessagingStyle("");
            //get all unread chats count
            int chatsCount = (int) net.trejj.rubytalksense.utils.RealmHelper.getInstance().getUnreadChatsCount();
            ...
            if (last7UnreadMessages.isEmpty()) {
                ...
                for (int i = 0; i < last7UnreadMessages.size(); i++) {
                    ...
                    if (chatHashMap.containsKey(chatId)) {
                        ...
                        chat = net.trejj.rubytalksense.utils.RealmHelper.getInstance().getChat(chatId);
                    ...
                    if (isNotificationsEnabled && !chat.isMuted()) {
                        ...
                        if (chat.getUser().isGroupBool()) {
                            ...
                            User user = net.trejj.rubytalksense.utils.ListUtil.getUserById(unreadMessage.getFromId(), users);
                            if (user != null) {
            ...
            List<Chat> unreadChats = net.trejj.rubytalksense.utils.RealmHelper.getInstance().getUnreadChats();
            ...
            for (Chat unreadChat : unreadChats) {
                ...
                if (isNotificationsEnabled && !unreadChat.isMuted()) {
                    ...
                    List<Message> messageList = net.trejj.rubytalksense.utils.RealmHelper.getInstance().filterUnreadMessages(unreadChat.getUnreadMessages());
                    if (isGroup) {
    ...
    private PendingIntent getPendingIntent(Chat chat) {
        ...
        if (isBelowApi24()) {
            ...
            if (chat != null) {
                ...
                intent.putExtra(net.trejj.rubytalksense.utils.IntentUtils.UID, chat.getChatId());
...
//        start chatActivity with the clicked user
    ...
    private Intent getMessageReplyIntent(String label, String chatId) {
        ...
        intent
                ...
                .setAction(net.trejj.rubytalksense.utils.IntentUtils.INTENT_ACTION_HANDLE_REPLY)
                .putExtra(KEY_PRESSED_ACTION, label)
                .putExtra(net.trejj.rubytalksense.utils.IntentUtils.UID, chatId)
                .putExtra(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_CHAT_ID, chatId);
        return intent;
    }
    ...
    private Intent getMarkAsReadIntent(String chatId, boolean isGroup) {
        ...
        intent
                .setAction(net.trejj.rubytalksense.utils.IntentUtils.INTENT_ACTION_MARK_AS_READ)
                .putExtra(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_CHAT_ID, chatId)
                .putExtra(net.trejj.rubytalksense.utils.IntentUtils.IS_GROUP, isGroup);
        return intent;
    }
    ...
    public Notification createActiveCallNotification(FireCall fireCall, int notificationId) {
        ...
        PendingIntent notificationPIntent = getNotificationClickPendingIntent(fireCall, net.trejj.rubytalksense.utils.IntentUtils.NOTIFICATION_ACTION_CLICK, PI_REQUEST_CODE_CLICK);
        ...
        Intent hangupIntent = getCallingActivityIntent(fireCall, net.trejj.rubytalksense.utils.IntentUtils.NOTIFICATION_ACTION_HANGUP);
    ...
    private Intent getCallingActivityIntent(FireCall fireCall, int action) {
        ...
        notificationIntent.putExtra(net.trejj.rubytalksense.utils.IntentUtils.CALL_DIRECTION, fireCall.getDirection());
        notificationIntent.putExtra(net.trejj.rubytalksense.utils.IntentUtils.CALL_TYPE, fireCall.getCallType());
        notificationIntent.putExtra(net.trejj.rubytalksense.utils.IntentUtils.CALL_ID, callId);
        notificationIntent.putExtra(net.trejj.rubytalksense.utils.IntentUtils.UID, fireCall.getUser().getUid());
        notificationIntent.putExtra(net.trejj.rubytalksense.utils.IntentUtils.PHONE, fireCall.getPhoneNumber());
        notificationIntent.putExtra(net.trejj.rubytalksense.utils.IntentUtils.CALL_ACTION_TYPE, action);
...
//        notificationIntent.addFlags(Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS);
    ...
    public Notification createIncomingCallNotification(FireCall fireCall, int notificationId) {
        PendingIntent notificationPIntent = getNotificationClickPendingIntent(fireCall, net.trejj.rubytalksense.utils.IntentUtils.NOTIFICATION_ACTION_START_INCOMING, PI_REQUEST_CODE_CLICK);
