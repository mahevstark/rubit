...
public class PBundleUtil {
    ...
    public PersistableBundle getBundle() {
        ...
        bundle.putString(net.trejj.rubytalksense.utils.IntentUtils.ID, id);
        if (messageId != null)
            bundle.putString(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_MESSAGE_ID, messageId);
        if (myUid != null)
            bundle.putString(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_MY_UID, myUid);
        if (stat != 0)
            bundle.putInt(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_STAT, stat);
        if (groupId != null)
            bundle.putString(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_GROUP_ID, groupId);
        if (groupEvent != null)
            bundle.putPersistableBundle(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_GROUP_EVENT, groupEventToPersistableBundle(groupEvent));
        if (chatId != null)
            bundle.putString(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_CHAT_ID, chatId);
        return bundle;
    }
    ...
    private PersistableBundle groupEventToPersistableBundle(GroupEvent groupEvent) {
        PersistableBundle bundle = new PersistableBundle();
        bundle.putString(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_CONTEXT_START, groupEvent.getContextStart());
        bundle.putInt(net.trejj.rubytalksense.utils.IntentUtils.EXTRA_EVENT_TYPE, groupEvent.getEventType());
