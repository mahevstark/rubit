...
public class GroupLinkUtil {
    ...
    private static String generateNewKey(String groupId) {
        return net.trejj.rubytalksense.utils.FireConstants.groupsLinks.child(groupId).push().getKey();
    }
        ...
        getCurrentLink(groupId, new FetchCurrentLinkCallback() {
            ...
            public void onFetch(String groupLink) {
                ...
                if (groupLink == null) {
                    ...
                    //delete old link
                    net.trejj.rubytalksense.utils.FireConstants.groupsLinks.child(groupLink).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
    ...
    private static void saveToDatabase(final String groupId, final String newKey, final GenerateLinkCallback callback) {
        net.trejj.rubytalksense.utils.FireConstants.groupLinkById.child(groupId).setValue(newKey)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        net.trejj.rubytalksense.utils.FireConstants.groupsLinks.child(newKey).setValue(groupId).addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                                if (task.isSuccessful()) {
    ...
    private static void getCurrentLink(final String groupId, final FetchCurrentLinkCallback callback) {
        net.trejj.rubytalksense.utils.FireConstants.groupLinkById.child(groupId).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
